...
import MyServiceCard2 from '../../components/Services/MyserviceCard2.jsx';
...
const MyBookings = () => {
    ...
    const { data: appointments, loading, error } = useFetchData(`${BASE_URL}/hairdressers/myAppointments`, {
        headers: {
    ...
    return (
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-5'>
            {appointments.map((booking, index) => (
                <MyServiceCard2 booking={booking} key={`${booking._id}-${index}`} />
            ))}
        </div>
    );
};
...
const Dashboard = () => {
    ...
    const { data: userData, loading, error } = useFetchData(
        user && user._id ? `${BASE_URL}/hairdressers/profile/${user._id}` : null,
        {
            headers: {
                Authorization: `Bearer ${user?.token}`
            }
    ...
    const handleDeleteAccount = async () => {
        if (window.confirm('Czy na pewno chcesz usunąć swoje konto?')) {
            try {
                const res = await fetch(`${BASE_URL}/hairdressers/${user._id}`, {
                    method: 'DELETE',
                    headers: {
            ...
            <div className='max-w-[1170px] px-5 mx-auto'>
                <div className='grid md:grid-cols-3 gap-10'>
                    ...
                    <div className='col-span-2 pb-[50px] px-[30px]'>
                        ...
                        {tab === 'bookings' ? <MyBookings /> : tab === 'profile' ? <Profile user={userData} /> : tab === 'users' ? <UserList /> : <Schedule user={userData} />}
                    </div>
                </div>
            </div>
        </section>
